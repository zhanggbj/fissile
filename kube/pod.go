	...
	for _, config := range configs {
		...
		if config.Secret {
			...
			if !settings.CreateHelmChart {
				...
				envVar.Set(helm.Block(fmt.Sprintf("if .Values.secrets.%s", util.ConvertNameToKey(config.Name))))
				env = append(env, envVar)
				...
				envVar := helm.NewMapping("name", config.Name, "valueFrom", helm.NewMapping("secretKeyRef", secretKeyRef))
				envVar.Set(helm.Block(fmt.Sprintf("if (not .Values.secrets.%s)", util.ConvertNameToKey(config.Name))))
